...
///
/// 수치 단위: 거리는 전부 <b>월드 유닛</b>이다. 이 65행 데이터는 <b>기획 문서의 픽셀 수치 ÷ 50</b>으로
/// 만들어졌는데, 2026-08-10 카메라를 FHD 기준(orthographicSize=5.4)으로 맞추면서
/// <b>화면의 실제 환산은 1유닛 = 100px</b>이 되었다(배경 스프라이트 PPU와 일치).
/// 즉 지금 데이터의 거리는 화면 기준으로 <b>의도한 것의 2배</b>이며, 이 어긋남은 아직 정리하지 않았다.
/// 참고로 직교 카메라 기준 가시 반경은 <b>세로 5.4 / 가로 9.6 유닛</b>이다
/// (교전이 화면 안에서 일어나도록 PlayerShootManager.max_detect_range=5.3이 감지거리를 한 번 더 자른다).
/// </summary>
[Serializable]
