...
///
/// 무기의 등급·가격은 구글시트 무기 테이블에 컬럼이 없어서 여기에 둔다. 시트를 건드리지 않는
/// 이유는 두 가지다: (1) 기획자 시트 스키마를 바꾸지 않아도 되고, (2) GameDataCsvImporter가
/// 컬럼을 <b>순서(인덱스)</b>로 읽기 때문에 시트 중간에 컬럼이 끼어들면 기존 파싱이 조용히
/// 어긋난다. 나중에 시트로 옮기기로 하면 이 에셋의 값을 그대로 컬럼으로 옮기면 된다.
///
/// 무기는 "종류 = 수평 강화 / 등급 = 수직 강화" 규칙에 따라, 시트에 있는 무기 6종을 그대로
/// 두고 등급별 배율(GradeSetting.statMultiplier)을 곱해서 상위 등급 무기를 만들어낸다.
