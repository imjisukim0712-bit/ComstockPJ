...
public class ItemCellLayout : MonoBehaviour
{
    ...
    /// 끊어서 하한 20pt까지 줄어든다). 이름표 폭이 150px 이상이면 "Weapon Socket 1"·"자기장 코어"가
    ...
    private void Apply()
    {
        ...
        if (innerWidth <= 4f || innerHeight <= 4f) return;               // 테두리가 칸보다 두껍다

        if (fill != null) SetInset(fill, left, bottom, right, top);

        bool hasCaption = caption != null;

        // 가로로 넓은 칸은 아이콘을 왼쪽·이름표를 오른쪽에 나란히 둔다(아래 UseSideBySide 참고).
        if (hasCaption && icon != null && UseSideBySide(innerWidth, innerHeight))
        {
    ...
    /// 유리하다</b> - 한 줄로 묶으면 "자기장 코어"가 폭에 맞춰 잘게 줄어들지만, 두 줄로 접으면
