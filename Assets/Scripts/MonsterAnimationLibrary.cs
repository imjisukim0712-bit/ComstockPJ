...
///
/// <b>캔버스 크기가 같아도 "그림 크기"는 다를 수 있다</b>(2026-08-24에 겪은 함정) - 같은
/// 800px 캔버스라도 그 안에 그려진 몬스터가 캔버스의 40%인 세트와 75%인 세트가 섞여 있으면
/// 세트를 바꿀 때 몸이 갑자기 커지거나 작아진다. 새 세트를 넣을 때는 캔버스 픽셀 수가 아니라
/// <b>알파 실루엣을 기준 세트와 겹쳐서(IoU) 실측한 배율</b>로 PPU를 맞추고, 그림이 캔버스
/// 안에서 치우쳐 있으면 pivot까지 옮겨야 한다(보스 돌진/차저 돌진준비 세트가 그랬다 -
/// <see cref="BossUnit"/> 클래스 주석 참고).
