...
public static class RewardPickupManager
{
    ...
    // 크기를 1/2로 줄여줘"에 따라 기준 크기를 0.6 → 0.3으로 내렸다. 세 아이콘 전부 이
    // 배율 하나만 쓰므로 상대 비율(상자:코인 1.667배)은 그대로 유지된다.
    // 2026-08-25 사용자 지정 - "드랍아이템들도 1.5배 크기로 키워줘"에 따라 0.3 → <b>0.45</b>.
    private const string ReferenceIconResourceName = "Gold";
    private const float ReferenceIconWorldSize = 0.45f;
